...
# # # =========================================================================
# # class Config:
# #     BEDROCK_KB_ID = os.getenv("BEDROCK_EMBEDDING_MODEL_ID")
...
# # # =========================================================================
# # # Conversation Workflow (Fixed)
# # # =========================================================================
# # class EducationAssistant:
# #     """Main conversation orchestrator with proper thread handling"""
...
# #         """Create state machine with proper checkpointer configuration"""
...
# #         return "fetch_knowledge" if state["intent"] == "education" else "generate_response"
...
# #     # Start conversation with fixed thread ID
...
# #         {"query": "What did we discuss about Stanford?", "history": history, "thread_id": thread_id},
# #         {"configurable": {"thread_id": thread_id}}
# #     )
...
# # =========================================================================
# class Config:
#     BEDROCK_KB_ID = "BVWGHMKJOQ"#os.getenv("BEDROCK_KNOWLEDGE_BASE_ID")
#     AWS_REGION = os.getenv("BEDROCK_REGION_NAME", "us-west-2")
#     EMBED_MODEL = "amazon.titan-embed-text-v2:0"  # Verified model
#     CHAT_MODEL = "meta.llama3-70b-instruct-v1:0"  # Verified model
#     MEMORY_SEARCH_K = 3
#     MAX_HISTORY = 5
#     SESSION_TIMEOUT = 300
...
#     def recall_memories(self, query: str, filters: dict, k: int = 3) -> List[str]:
#         """Retrieve relevant memories with metadata filtering"""
#         def _filter(doc: Document) -> bool:
#             return all(doc.metadata.get(k) == v for k, v in filters.items())
...
#     def _route_based_on_intent(self, state: dict):
#         """Determine processing path based on intent"""
#         return state["intent"]
...
#             print(f"Bot: {memory_test['response']}")
#         except Exception as e:
#             print(f"Memory recall failed: {str(e)}")




"""
AWS Bedrock Educational Chatbot - Stable Version 4.1
"""
    ...
    def recall_memories(self, query: str, filters: dict, k: int = 5) -> List[str]:
        try:
            ...
            def _filter(doc: Document) -> bool:
                ...
                ) and query.lower() in doc.page_content.lower()

            docs = self.vector_store.similarity_search(query, k=k, filter=_filter)
            
            # Fallback to exact match
            if not docs:
                docs = [doc for doc in self.vector_store._collection 
                        if _filter(doc)]
            
            logger.info(f"Recalled {len(docs)} memories for query: {query}")
...
class EducationAssistant:
    ...
    def _retrieve_context(self, state: dict):
        ...
        logger.debug(f"Context retrieved: {state['memories']}")
        return state
    ...
    def _fetch_knowledge(self, state: dict):
        ...
        if Config.BEDROCK_KB_ID:
            try:
                ...
                logger.info(f"Retrieved {len(state['documents'])} knowledge documents")
            except Exception as e:
                logger.error(f"Knowledge retrieval failed: {str(e)}")
        return state
...
if __name__ == "__main__":
    ...
    test_queries = [
        ...
        "What's the admission process for Stanford CS?",
        "What about scholarship opportunities?",
        "What did we discuss about Stanford?"
    ]
